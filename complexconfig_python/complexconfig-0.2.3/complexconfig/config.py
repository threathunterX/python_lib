...
class Config(object):
    ...
    def normalize_config(self, config):
        """
        normalize the config from parser

        the raw config should be normalized and make the dictionary has depth 1
        :param config:
        :return: config with depth 1
        """
        result_dict = dict()
        self._normalize_config(config, '', result_dict)
        return result_dict

    def _normalize_config(self, current_node, prefix, result_dict):
        """
        internal method for normalize_config
        :param current_node: current dictionary_node
        :param prefix: current prefix
        :param result_dict: the generated config should be inserted to result_dict
        :return:
        """
        ...
        import collections
        if isinstance(current_node, collections.Mapping):
            ...
            for key, value in current_node.iteritems():
                if prefix:
                    ...
                    new_prefix = key
                self._normalize_config(value, new_prefix, result_dict)
        else:
            result_dict[prefix] = current_node
    ...
    def _load_config(self):
        ...
        try:
            ...
            # 2. normalize
            self.config_data = self.normalize_config(self.config_data)

            # 3. mark we have config now
    ...
    def is_valid(self):
        """
        tell if the config is in a correct state
        :return: true if the config has been loaded correctly
        """
        return self.has_loaded

    def is_empty(self):
        """
        tell if the config data is empty
        :return:
        """
...
class MockConfig(Config):
    ...
    def __init__(self, data):
        ...
        self.data = self.normalize_config(self.data)
