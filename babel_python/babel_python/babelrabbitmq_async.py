...
def get_client_sender_for_queue(service_meta, **kwargs):
    ...
    ex = "_queue"
    return KombuQueueSender(get_sdc(service_meta), ex, "direct", **kwargs)
...
def get_server_receiver_for_queue(service_meta, **kwargs):
    ...
    ex = "_queue"
    return KombuQueueReceiver(get_sdc(service_meta), ex, "direct", service_meta.name, service_meta.name, **kwargs)
...
def get_client_sender_for_topic(service_meta, **kwargs):
    ...
    decide_durable(service_meta, kwargs)
    ex = "_topic"
    return KombuQueueSender(get_sdc(service_meta), ex, "topic", **kwargs)
...
def get_server_receiver_for_topic(service_meta, **kwargs):
    ...
    decide_durable(service_meta, kwargs)
    ex = "_topic"
    return KombuQueueReceiver(get_sdc(service_meta), ex, "topic", "{}.{}".format(service_meta.name, subname), service_meta.name, **kwargs)
#    return KombuQueueReceiver(get_sdc(service_meta), ex, "topic", service_meta.name, service_meta.name, **kwargs)
...
def get_client_sender_for_sharding(service_meta, **kwargs):
    ...
    return KombuQueueSender(get_sdc(service_meta), ex, "x-consistent-hash", **kwargs)
...
def get_server_receiver_for_sharding(service_meta, **kwargs):
    ...
    decide_durable(service_meta, kwargs)
    ex = "_sharding.{}".format(service_meta.name)
    return KombuQueueReceiver(get_sdc(service_meta), ex, "x-consistent-hash",
...
def get_client_sender_for_topic_sharding(service_meta, **kwargs):
    ...
    return KombuQueueSender(get_sdc(service_meta), ex, "fanout", **kwargs)
...
def get_server_receiver_for_topic_sharding(service_meta, **kwargs):
    ...
    decide_durable(service_meta, kwargs)
    ex = "_toshard.{}".format(service_meta.name)
    return KombuTwoLayerQueueReceiver(get_sdc(service_meta), ex, "fanout", "*",
...
def get_client_receiver_for_rpc_client(cdc, clientid, **kwargs):
    ...
    return KombuQueueReceiver([cdc], "_client", "direct", clientid, clientid, **kwargs)


def get_server_sender_for_rpc_server(cdc, clientid,service_meta_name, **kwargs):
    kwargs["queue_durable"] = False
    return KombuQueueSender([cdc], "_client", "direct", **kwargs)
...
class RabbitmqClientSender(object):
    ...
    def get_sharding_key(self, key):
        if self.service_meta.delivermode in ["sharding", "topicsharding"]:
            cardinality = self.service_meta.options.get("servercardinality", 1)
            cardinality = min(cardinality, 20)
            return str(SHARD_DATA[get_hash(key) % cardinality])
        else:
            return key
