...
class TestService:
    ...
    def init_services():
        ...
        TestService.build_service("service_notify_toshuffle1", "service_notify_toshuffle", "notify", "topicshuffle", "consumer1", 1, 2)
        TestService.build_service("service_notify_toshuffle2", "service_notify_toshuffle", "notify", "topicshuffle", "consumer1", 2, 2)
        TestService.build_service("service_notify_toshuffle3", "service_notify_toshuffle", "notify", "topicshuffle", "consumer2", 1, 2)
        #
        #  3 services use topic sharding, 2 for consumer1, 1 for consumer2
        TestService.build_service("service_notify_tosharding1", "service_notify_tosharding", "notify", "topicsharding", "consumer1", 1, 2)
        TestService.build_service("service_notify_tosharding2", "service_notify_tosharding", "notify", "topicsharding", "consumer1", 2, 2)
        TestService.build_service("service_notify_tosharding3", "service_notify_tosharding", "notify", "topicsharding", "consumer2", 1, 2)
        ...
        #
        #  3 services use topic shuffle, 2 for consumer1, 1 for consumer2
        TestService.build_service("service_polling_toshuffle1", "service_polling_toshuffle", "polling", "topicshuffle", "consumer1", 1, 2)
        TestService.build_service("service_polling_toshuffle2", "service_polling_toshuffle", "polling", "topicshuffle", "consumer1", 2, 2)
        TestService.build_service("service_polling_toshuffle3", "service_polling_toshuffle", "polling", "topicshuffle", "consumer2", 1, 2)
        #
        #  3 services use topic sharding, 2 for consumer1, 1 for consumer2
        TestService.build_service("service_polling_tosharding1", "service_polling_tosharding", "polling", "topicsharding", "consumer1", 1, 2)
        TestService.build_service("service_polling_tosharding2", "service_polling_tosharding", "polling", "topicsharding", "consumer1", 2, 2)
        TestService.build_service("service_polling_tosharding3", "service_polling_tosharding", "polling", "topicsharding", "consumer2", 1, 2)
    ...
    def test_notify_topic_shuffle(self):
        if TestService.CLIENT:
            self.client = ServiceClient(TestService.server_repo["service_notify_toshuffle1"]["service"])
            self.client.start()
            for i in range(20):
                ev = TestService.EVENT.copy()
                ev.key = str(i)
        ...
        if TestService.CLIENT and TestService.SERVER:
            sleep(1)
            results1 = TestService.server_repo["service_notify_toshuffle1"]["results"]
            results2 = TestService.server_repo["service_notify_toshuffle2"]["results"]
            results3 = TestService.server_repo["service_notify_toshuffle3"]["results"]
    ...
    def test_notify_topic_sharding(self):
        if TestService.CLIENT:
            self.client = ServiceClient(TestService.server_repo["service_notify_tosharding1"]["service"])
            self.client.start()
            for i in range(20):
                ev = TestService.EVENT.copy()
                ev.key = str(i)
        ...
        if TestService.CLIENT and TestService.SERVER:
            sleep(1)
            results1 = TestService.server_repo["service_notify_tosharding1"]["results"]
            results2 = TestService.server_repo["service_notify_tosharding2"]["results"]
            results3 = TestService.server_repo["service_notify_tosharding3"]["results"]
            assert len(results1) + len(results2) == 20
            assert len(results3) == 20
    ...
    def test_rpc_topic_shuffle(self):
        if TestService.CLIENT:
            self.client = ServiceClient(TestService.server_repo["service_polling_toshuffle1"]["service"])
            self.client.start()
            result = self.client.send(TestService.EVENT, "", True, 1)
            assert result == (True, [TestService.EVENT, TestService.EVENT])

        if TestService.CLIENT and TestService.SERVER:
            results1 = TestService.server_repo["service_polling_toshuffle1"]["results"]
            results2 = TestService.server_repo["service_polling_toshuffle2"]["results"]
            results3 = TestService.server_repo["service_polling_toshuffle3"]["results"]
            assert len(results1) + len(results2) == 1
    ...
    def test_rpc_topic_sharding(self):
        if TestService.CLIENT:
            self.client = ServiceClient(TestService.server_repo["service_polling_tosharding1"]["service"])
            self.client.start()
            result = self.client.send(TestService.EVENT, "", True, 1)
            assert result == (True, [TestService.EVENT, TestService.EVENT])

        if TestService.CLIENT and TestService.SERVER:
            results1 = TestService.server_repo["service_polling_tosharding1"]["results"]
            results2 = TestService.server_repo["service_polling_tosharding2"]["results"]
            results3 = TestService.server_repo["service_polling_tosharding3"]["results"]
            assert len(results1) + len(results2) == 1
            assert len(results3) == 1
    ...
    def _performance_test_one_thread(self, client, round, service):
        if TestService.CLIENT:
            for i in xrange(round):
                result = client.send(TestService.EVENT, "", False, 5)
                if not result[0]:
    ...
    def test_notify_queue_performance_single_thread(self):
        ...
        if TestService.CLIENT:
            ...
            if TestService.CLIENT and TestService.SERVER:
                results = TestService.server_repo["service_notify_queue"]["results"]
                current = millis_now()
                while len(results) != round and (current - start) <= 20000:
                    sleep(0.1)
                    current = millis_now()
