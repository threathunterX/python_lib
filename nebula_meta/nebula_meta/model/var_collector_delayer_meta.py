...
class DelayCollectorVariableMeta(VariableMeta):
    """
    only difference from VarCollectorVariableMeta is:
    - class type
    - sleep in config
    - condition in config always be None
    """
    ...
    def __init__(self, *args, **kwargs):
        kwargs["type"] = DelayCollectorVariableMeta.TYPE
        ...
        self.sleep_duration = int(kwargs["sleep_duration"]) * int(UNIT_MAP[kwargs["sleep_unit"]])
    ...
    def from_dict(d):
        ...
        return DelayCollectorVariableMeta(**kwargs)

    @staticmethod
    def from_json(jsonStr):
        return DelayCollectorVariableMeta.from_dict(json.loads(jsonStr))

    def copy(self):
        return DelayCollectorVariableMeta.from_dict(self.get_dict())

    def __str__(self):
        return "DelayCollectorVariableMeta[{}]".format(self.get_dict())
