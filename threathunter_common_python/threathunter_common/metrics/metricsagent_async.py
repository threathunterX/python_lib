...
class MetricsAgentAsync(object):
    _INSTANCE = None
    _LOCK = Semaphore(1)
    ...
    @staticmethod
    def get_instance():
        if MetricsAgentAsync._INSTANCE is None:
            with MetricsAgentAsync._LOCK:
                if MetricsAgentAsync._INSTANCE is None:
                    MetricsAgentAsync._INSTANCE = MetricsAgentAsync()
        return MetricsAgentAsync._INSTANCE
    ...
    def initialize_by_dict(self, data, db=None, server_name=None):
        with MetricsAgentAsync._LOCK:
            ...
            self.running = True
            self.task = gevent.spawn(self.backend_task)
