...
class Connection(object):
    ...
    def execute(self, request, connection_para=None):
        ...
        while True:
            ...
            try:
                """
                Note: tsli modified here
                Project: Athena
                Purpose: for connecting ES of OPS and Big Security via token mode
                """
                if connection_para:
                    conn = get_pool().connection_from_host(connection_para["hostname"], connection_para["port"], connection_para["scheme"])
                    kwargs["body"] = str({"request_body": kwargs["body"], "access_token": connection_para["token"]})
                    kwargs["method"] = connection_para["method"]
                    kwargs["url"] = connection_para["url_prefix"] + kwargs["url"]
                else:
                    parse_result = urlparse(server)
                    conn = get_pool().connection_from_host(parse_result.hostname, parse_result.port, parse_result.scheme)
