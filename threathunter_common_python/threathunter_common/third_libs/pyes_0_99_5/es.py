...
class ES(object):
            ...
            if isinstance(server, (tuple, list)):
                ...
                if server.startswith(("thrift:", "http:", "https:")):
                    ...
                    if len(tokens) == 2:
                        ...
                        # tsli modified here: 大安ES的端口是443，所以专门加一个443端口为http访问模式
                        if 9200 <= port <= 9299 or 443==port:
                            _type = "http"
                        elif 9500 <= port <= 9599:
                            _type = "thrift"
                        else:
    ...
    def _send_request(self, method, path, body=None, params=None, headers=None, raw=False, return_response=False, connection_para=None):
        if params is None:
            params = {}
        ...
        # execute the request
        response = self.connection.execute(request, connection_para=connection_para)
    ...
    def ensure_index(self, index, mappings=None, settings=None, clear=False):
        ...
        if exists:
            ...
            if isinstance(mappings, SettingsBuilder):
                ...
                from threathunter_common.third_libs.pyes_0_99_5.mappings import DocumentObjectField, ObjectField
    ...
    def partial_update(self, index, doc_type, id, script, doc=None, params=None,
    ...
    def search_raw(self, query, indices=None, doc_types=None, headers=None, connection_para=None, **query_params):
        ...
        return self._send_request('GET', path, body, params=query_params, headers=headers, connection_para=connection_para)
    ...
    def search(self, query, indices=None, doc_types=None, model=None, scan=False, headers=None, connection_para=None, **query_params):
        ...
        return ResultSet(self, search, indices=indices, doc_types=doc_types,
                         model=model, query_params=query_params, headers=headers, connection_para=connection_para)
...
class ResultSet(object):
    def __init__(self, connection, search, indices=None, doc_types=None, query_params=None,
                 headers=None, auto_fix_keys=False, auto_clean_highlight=False, model=None, connection_para=None):
        """
        results: an es query results dict
        fix_keys: remove the "_" from every key, useful for django views
        clean_highlight: removed empty highlight
        ...
        if search.bulk_read is not None:
            ...
            self.chuck_size = 10
        self.connection_para = connection_para
    ...
    def _do_search(self, auto_increment=False):
        ...
        if self.scroller_id is None:
            ...
            #真正查询函数
            self._results = self._search_raw(self.start, self.chuck_size, self.connection_para)
    ...
    def _search_raw(self, start=None, size=None, connection_para=None):
        if start is None and size is None:
            query_params = self.query_params
        else:
            query_params = dict(self.query_params)
        ...
        return self.connection.search_raw(self.search, indices=self.indices,
                                          doc_types=self.doc_types, headers=self.headers, connection_para=connection_para, **query_params)
